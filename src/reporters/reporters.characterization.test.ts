...
describe('reporter output compatibility', () => {
  ...
  it('preserves Console chunks, fallbacks, blank lines, and vector output', () => {
    ...
    expect(log.mock.calls).toEqual(
      [
        ...
        '   Found 1 potential semantic mismatches:\n',
        '   ⚠️  docs/api.md',
        '      Section: "Usage"',
        '      Similarity: 12.5%',
        '      Best match: -',
    ...
    expect(log.mock.calls.slice(-8)).toEqual(
      [
        '🔍 Semantic Analysis (Vector Search):\n',
        '   Found 1 potential semantic mismatches:\n',
        '   ⚠️  docs/api.md',
  ...
  it('preserves JSON bytes, key order, indentation, timestamp, and score modes', () => {
    ...
    expect(reporter.generateWithScores(cleanResults, null)).toBe(
      JSON.stringify({ ...cleanResults, summary: { ...cleanResults.summary, info: 0 }, freshnessScores: null, generatedAt }, null, 2)
    );
    expect(reporter.generateWithScores(cleanResults, scores)).toBe(
      JSON.stringify({ ...cleanResults, summary: { ...cleanResults.summary, info: 0 }, freshnessScores: scores, generatedAt }, null, 2)
    );
  });
...
|----------|---------|------------|------------|------------|
| docs/api.md | Usage | 12.5% | - | Review this section |
