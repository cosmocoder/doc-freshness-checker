...
export function createEnhancedReportContext(
  ...
  for (const document of results.documents) {
    ...
    for (const issue of document.issues) {
      ...
      const severity = isError ? '❌' : isInfo ? 'ℹ️' : '⚠️';
      issues.push(
        Object.freeze({
  ...
  const semanticAnalysis = formatVectorMismatches(results.vectorMismatches);
...
export function renderEnhancedReport(report: EnhancedReportContext): string {
  ...
  output += `- **Info:** ${enhanced.summary.info}\n\n`;
  ...
  output += enhanced.semanticAnalysis;
