...
describe('FileValidator', () => {
  ...
  it('handles pre-marked illustrative references', async () => {
    const results = await validator.validateBatch(
      [makeRef('real-looking-path.ts', { isIllustrative: true })],
      makeDoc(),
      config
    );
    expect(results[0].skipped).toBe(true);
  ...
  it('returns null suggestion when file directory does not exist', async () => {
    const results = await validator.validateBatch(
      [makeRef('zzz-nonexistent-dir/some-file.ts')],
      makeDoc(),
      { ...config, rootDir: process.cwd() }
    );
    expect(results[0].valid).toBe(false);
  ...
  it('reports illustrative path escaping project root', async () => {
    ...
    const results = await validator.validateBatch(
      [makeRef('/etc/YourProject/file.ts')],
      makeDoc(),
      noSkipConfig
    );
    expect(results[0].valid).toBe(false);
  ...
  it('uses custom illustrative patterns from config', async () => {
    ...
    const results = await validator2.validateBatch(
      [makeRef('custom-prefix-file.ts')],
      makeDoc(),
      customConfig
    );
    expect(results[0].skipped).toBe(true);
  ...
  it('caches directory listings for repeated lookups', async () => {
    const validator2 = new FileValidator();
    await validator2.validateBatch(
      [makeRef('../package-typo.json')],
      makeDoc(),
      { ...config, rootDir: process.cwd() }
    );
    const results = await validator2.validateBatch(
      [makeRef('../another-typo.json')],
      makeDoc(),
      { ...config, rootDir: process.cwd() }
    );
    expect(results[0].valid).toBe(false);
